...
def handle_ingest(args: argparse.Namespace, cfg: Config) -> bool:
    ...
    try:
        vectorstore = build_vectorstore(
            ...
            persist_path=args.persist or cfg.persist_path,
...
def handle_query(args: argparse.Namespace, cfg: Config) -> bool:
    ...
    """
    log_info(f"Querying knowledgebase with question: {args.question}")

    try:
        answer = query_knowledgebase(
            query=args.question,
            persist_path=args.persist or cfg.persist_path,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Examples:\n"
        "  %(prog)s ingest --path ./documents\n"
        "  %(prog)s query --question 'What is the main topic?'\n"
        "  %(prog)s models",
